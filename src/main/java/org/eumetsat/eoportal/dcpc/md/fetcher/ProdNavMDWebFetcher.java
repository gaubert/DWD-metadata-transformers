...
public class ProdNavMDWebFetcher
{
    ...
    public File fetch() throws Exception
    {
        ...
        File topTempDir = FileSystem.createTempDirectory("download-",
                this.m_WorkingDir);
        ...
        if (oList.size() != 1)
        {
            throw new Exception("Error cannot find " + P1XPATH + " in " + url);
        }
        ...
        // need to be verbose otherwise non supported by java 1.5 (deferencement
        // issue)
        ...
        File destination = new File(topTempDir + File.separator
                + "downloaded_data.zip");
        FileOutputStream fOut = new FileOutputStream(destination);

        IOUtils.copy(in, fOut);

        logger.info("Retrieved {} from the Product Navigator.", destination
                .getName());
